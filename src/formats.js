...
const mp3 = {
  name: 'mp3',
  ext: 'mp3',
  desc: 'mp3 (mpeg-1 Audio Layer 3) is a music format that can compress files by up to 95%.',
  shortNames: ['mp3'],
  filePostfix: '',
  args: [],
}
...
export const parseFormatArg = (arg) => {
  const available = FORMATS.map((format) => [
    format.name,
    ...format.shortNames,
  ]).flat()
...
export const formatInfo = (name) => {
  const info = FORMATS.find(
    (format) => format.name === name || format.shortNames.includes(name),
  )
  return info
}
